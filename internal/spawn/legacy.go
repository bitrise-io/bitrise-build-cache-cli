...
// CLI versions up to v3.6.9 registered these services with the OS supervisor,
// and such a registration outlives the upgrade that stopped writing it.
const (
...
// RemoveLegacySupervision deletes any leftover LaunchAgent or systemd unit for
// svc. Best-effort: it reports whether it removed one, never why it could not.
func RemoveLegacySupervision(ctx context.Context, p paths.Paths, svc Service) bool {
	...
	case "linux":
		...
		//nolint:dogsled // best-effort: an already-gone unit is the outcome we want
