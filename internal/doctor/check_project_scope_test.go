...
func TestProjectScopeCheck_scenarioC_missingSlugIsWarnWithSeedHint(t *testing.T) {
	dir := t.TempDir()
	writeMarker(t, dir, `{"workspace":"acme"}`)
	t.Chdir(dir)
	...
	res := d.projectScopeCheck().Diagnose(context.Background())
	assert.Equal(t, StateWarn, res.State, "misconfigured C: marker slug not in the store's Workspaces map")
	assert.Contains(t, res.Detail, "auth set")
	assert.Contains(t, res.Detail, "--workspace-id acme")
}
