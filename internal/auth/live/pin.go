...
// starts can find it in a shell that never saw those env vars. A write failure is
// logged, not returned: the credential resolved fine and the activation should proceed.
//
// A workspaces-only store is a success with a zero credential: the per-build
// resolver looks up the token by workspace slug at run time.
func (r *Resolver) ResolvePinned(ctx context.Context, envs map[string]string, isCI bool) (auth.Credential, auth.Origin, error) {
	...
	if err != nil && r.storeHasWorkspacesOnly(envs) {
		return auth.Credential{}, auth.Origin{}, nil
	}
