...
func TestCallbackServer_HappyPath(t *testing.T) {
	cs, err := newCallbackServer(context.Background(), "st8")
	if err != nil {
	...
	go func() {
		resp, err := http.Get(cs.redirectURI() + "?code=abc&state=st8") //nolint:noctx,gosec
		if err == nil {
			_ = resp.Body.Close()
		}
	}()
...
func TestCallbackServer_StateMismatch(t *testing.T) {
	cs, err := newCallbackServer(context.Background(), "right")
	if err != nil {
		t.Fatalf("newCallbackServer: %v", err)
	}
	defer cs.close()
	...
	go func() {
		resp, err := http.Get(cs.redirectURI() + "?code=abc&state=wrong") //nolint:noctx,gosec
		if err == nil {
			_ = resp.Body.Close()
		}
	}()
...
func TestCallbackServer_Timeout(t *testing.T) {
	cs, err := newCallbackServer(context.Background(), "s")
	if err != nil {
		t.Fatalf("newCallbackServer: %v", err)
	}
	defer cs.close()
