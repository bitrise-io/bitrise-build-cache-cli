	...
	tests := []struct {
		...
		},
		{
			...
			expectContains: []string{
				...
				`val xlogTags = if (testJvmMajor >= 17) "class+load=info,library=info" else "class+load=info"`,
				`jvmArgs("-Xlog:${xlogTags}:file=${probeLog.getAbsolutePath()}::filesize=16m,filecount=1")`,
