...
// mirrors check_gradle_jwt_auth.sh: no authToken literal must survive on CI+empty-token.
func Test_GenerateInitGradle_CIWithEmptyAuthToken_OmitsAuthTokenLines(t *testing.T) {
	inv := TemplateInventory{
		Common: PluginCommonTemplateInventory{
			AuthToken:  "", // resolver saw only a JWT — commonTemplateInventory blanked it
			CIProvider: "bitrise",
			CLIPath:    "/opt/bitrise-build-cache",
			Version:    "commonV",
		},
		...
		Analytics: AnalyticsTemplateInventory{
			...
			Port:         443,
			HTTPEndpoint: "analytics-http",
			GRPCEndpoint: "analytics-grpc",
		},
		TestDistro: TestDistroTemplateInventory{
			Usage:      UsageLevelEnabled,
			Version:    "testDistroV",
			Endpoint:   "testDistroEndpoint",
			KvEndpoint: "testDistroKv",
			LogLevel:   "info",
		},
	}
	...
	assert.NotContains(t, got, "authToken", "check_gradle_jwt_auth.sh greps for any authToken occurrence in the emitted init script and fails if one is present")
}
