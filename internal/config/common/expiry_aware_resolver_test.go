...
func TestExpiryAwareResolver_NonOAuthSource_UsesPlainResolve(t *testing.T) {
	...
	refresh := func(_ context.Context) (string, string, error) {
		t.Fatal("refreshFn must not be called when the source is not store-managed")
...
func TestExpiryAwareResolver_FileSource_UsesRefreshFn(t *testing.T) {
	...
	resolve := func(_ map[string]string) (CacheAuthConfig, AuthSource, error) {
		return storedCfg, AuthSourceFile, nil
	}
	refresh := func(_ context.Context) (string, string, error) {
...
// The legacy authConfig block has no refresh token, so there is nothing to refresh.
func TestExpiryAwareResolver_MultiplatformSource_UsesPlainResolve(t *testing.T) {
	storedCfg := CacheAuthConfig{AuthToken: "legacy-tok", WorkspaceID: "legacy-ws"}
	resolve := func(_ map[string]string) (CacheAuthConfig, AuthSource, error) {
		return storedCfg, AuthSourceMultiplatform, nil
	}
	refresh := func(_ context.Context) (string, string, error) {
		t.Fatal("refreshFn must not be called for the legacy authConfig source")
