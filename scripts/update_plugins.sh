...
if [[ $latest_version =~ ^[0-9]+\.[0-9]+\.[0-9]+$ ]]; then
    ...
    if compare_versions "$current_version" "$latest_version"; then
        sed -i "s/GradleAnalyticsPluginDepVersion = \".*\"/GradleAnalyticsPluginDepVersion = \"$latest_version\"/" "$VERSION_FILE"
        sed -i "s/classpath(\"io.bitrise.gradle:gradle-analytics:.*\")/classpath(\"io.bitrise.gradle:gradle-analytics:$latest_version\")/" "$TEST_FILE"
...
if [[ $latest_version =~ ^[0-9]+\.[0-9]+\.[0-9]+$ ]]; then
    # Compare versions and update if the latest is greater
    if compare_versions "$current_version" "$latest_version"; then
        sed -i "s/GradleRemoteBuildCachePluginDepVersion = \".*\"/GradleRemoteBuildCachePluginDepVersion = \"$latest_version\"/" "$VERSION_FILE"
        sed -i "s/classpath(\"io.bitrise.gradle:remote-cache:.*\")/classpath(\"io.bitrise.gradle:remote-cache:$latest_version\")/" "$TEST_FILE"
        echo "Updated to version $latest_version"
    else
        echo "No update needed. Current version ($current_version) is up-to-date or newer."
    fi
