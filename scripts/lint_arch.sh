...
# 2. auth.TokenSet stays at or below L4. The credential-write surfaces are the
#    documented exception: they persist a login, which is a whole TokenSet.
hits=$(grep -rln 'auth\.TokenSet\|authpkg\.TokenSet' --include='*.go' . 2>/dev/null |
	grep -v '^./internal/auth/' |
	grep -v '^./internal/config/multiplatform/' |
	grep -v '_test\.go$' |
	grep -v '^./cmd/auth/' |
	grep -v '^./cmd/common/interactive/' |
	grep -v '^./internal/authprompt/' || true)
