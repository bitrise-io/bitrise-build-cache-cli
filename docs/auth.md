...
Four scenarios cover how a machine is set up. The `auth status` command prints
the current scenario as `Scenario: <name>` so a support session doesn't need to
guess.

| Scenario | Store shape                                              | `auth set` command                                                                 | `auth status` output              |
|----------|----------------------------------------------------------|------------------------------------------------------------------------------------|-----------------------------------|
| A        | Machine-wide slot populated. No `Workspaces` map.        | `auth set --token <t> --workspace-id <ws>`                                         | `Scenario: machine-wide only`     |
| B        | `Workspaces` map has one or more entries. Top-level empty. | `auth set --token <t> --workspace-id <slug>` (repeatable per slug)                | `Scenario: per-workspace only`    |
| C        | Both. Machine-wide is the fallback when no marker matches. | `auth set --token <t> --workspace-id <ws>` then `auth set --token <t2> --workspace-id <slug>` | `Scenario: both (machine-wide as fallback)` |
| None     | Nothing stored, nothing in env.                          | (any of the above)                                                                 | `Scenario: none`                  |

**Scenario B** is the shape a fully per-project setup lives in: `activate gradle`
et al. treat a workspaces-only store as "credential resolves per build", write no
auth into the generated config, and every build tool's runtime path (Gradle
ValueSource, Bazel credential helper, Xcode wrapper, ccache proxy) looks the
credential up by the marker's workspace slug.

**Scenario C** is the safe migration path. The marker's slug decides which
per-workspace token applies. When the marker names a slug that is not in
`Workspaces`, the doctor's `project-scope` check WARNs with a seed command
(`auth set --token <t> --workspace-id <slug>`) and the build falls back to the
machine-wide credential.

`ResolveNoRefresh` returns a `workspacesOnly` boolean alongside the credential;
every activate site treats `workspacesOnly=true` as "skip baking auth into
config; the runtime path will resolve it". The predicate is: no top-level
`AuthToken` on any backend, no env vars, at least one backend has a non-empty
`Workspaces` map. `ResolvePinned` short-circuits to a zero credential in the
same state — nothing to persist since the resolver has nothing yet.
