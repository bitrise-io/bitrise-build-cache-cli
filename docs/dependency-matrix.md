...
metadata), pin the activating step to a full patch version in your
workflow. The matching CLI version’s release page also has
`verification-metadata.xml` (and `verification-metadata-mirror.xml` from
v2.4.6 onward, for builds where the Bitrise Maven Central mirror is
active) attached as assets — use it to seed your
