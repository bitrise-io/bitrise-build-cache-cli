...
- In any other CI environment or for local development:
  ...
  - **Or, instead of the two variables above, just run `bitrise-build-cache login`** once. It signs you in through the browser, lets you pick a workspace, and stores an auto-refreshing token that subsequent commands use automatically — no manual token to create or env vars to set. Run `bitrise-build-cache logout` to clear it. This is for local development (it needs a browser on the same machine as the CLI). A manually-set `BITRISE_BUILD_CACHE_AUTH_TOKEN`, and the auto-provided token on Bitrise CI, both still take precedence over a stored login.
