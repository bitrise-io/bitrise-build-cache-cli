...
var activateForGradleParams = ActivateForGradleParams{
	Cache: CacheParams{
		Enabled:         false,
...
func activateForGradleCmdFn(logger log.Logger, gradleHomePath string, envProvider func(string) string) error {
	...
	testDistroAppSlug := os.Getenv("BITRISE_APP_SLUG")
	...
	if testDistroAppSlug == "" && testDistroUsageLevel == gradleconfig.UsageLevelEnabled {
		logger.Errorf("No env variable was provided for BITRISE_APP_SLUG, test distribution plugin will be added only as a dependency.")
		testDistroUsageLevel = gradleconfig.UsageLevelDependency
	}
