...
var authSetCmd = &cobra.Command{
	...
	RunE: func(_ *cobra.Command, _ []string) error {
		...
		switch scrubbed, err := scrubDiskCredentials(target.Backend()); {
		case err != nil:
			logger.Warnf("Saved to %s, but could not strip plain-text credentials from disk: %v", target.Backend(), err)
...
func init() {
	...
	authSetCmd.Flags().StringVar(&setWorkspaceID, "workspace-id", "", "Bitrise workspace ID (required). The token is saved under this slug in the per-workspace map (readable via `auth token --workspace=<slug>`).")
