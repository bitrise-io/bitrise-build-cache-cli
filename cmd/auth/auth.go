...
var authSetCmd = &cobra.Command{
	...
	RunE: func(_ *cobra.Command, _ []string) error {
		...
		target, err := store.Select(configcommon.IsCI(utils.AllEnvs(), utils.DefaultOsProxy{}), setStorage)
		if err != nil {
		...
		if result.Origin.Backend == authpkg.BackendFile {
			...
			if configcommon.IsCI(utils.AllEnvs(), utils.DefaultOsProxy{}) && setStorage == "" {
...
func setLocalUsername(envs map[string]string, name string) error {
	...
	origin, err := store.SetUsername(configcommon.IsCI(envs, utils.DefaultOsProxy{}), name)
	if err != nil {
