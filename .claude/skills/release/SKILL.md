...
- Verify the GitHub release has all 8 expected assets, the GAR mirror uploads (binaries + checksums + `installer.sh:<tag>` + `installer.sh:latest-pointer:{installer.sh,VERSION}`) succeeded, and the `Post-release verification` step (running `scripts/verify_release.sh`) passed — all critical.
