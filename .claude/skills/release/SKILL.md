...
5. **Step GitHub releases** — cut for the scoped steps (Step 8). **Expected count = scoped_steps + 1 for each dual-publish repo included in the scope** (xcode and RN produce BOTH `0.x` and `1.x` tags when scoped in).
6. **Steplib PRs** — merged (Step 9). **Expected count matches Step 8** — 2 steplib PRs per scoped-in dual-publish repo (one per tag).
...
- **Treat any failure of the CLI `release` workflow as a drop-everything-and-fix incident.** Don't move on to step releases until the CLI release workflow is green AND the release has all 8 expected assets (5 platform tarballs, checksums.txt, both verification XMLs).
